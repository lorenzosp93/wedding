<template>
  <div class="z-20 absolute left-0 top-0 backdrop-blur-sm h-screen w-screen" @click="$emit('toggle')">
  </div>
  <div
 ...
 rounded-lg bg-pale dark:bg-darkPale ring-1 ring-accent p-3 w-full max-w-sm">
    <form class="flex flex-wrap" @submit="setupPlusOne">
      <div class="my-1 px-5 mx-auto w-full text-start">
        <label class="w-full" for="first_name">{{ $t('profile.plusone.first_name') }}</label>
        <input id="first_name" v-model.trim="submit.first_name" class="w-full rounded-md bg-neutral dark:bg-darkNeutral" type="text">
        <p v-if="error?.first_name" class="text-alert mx-3">{{ error.first_name[0] }}</p>
      </div>
      <div class="my-1 px-5 mx-auto w-full text-start">
        <label class="w-full" for="last_name">{{ $t('profile.plusone.last_name') }}</label>
        <input id="last_name" v-model.trim="submit.first_name" class="w-full rounded-md bg-neutral dark:bg-darkNeutral" type="text">
        <p v-if="error?.last_name" class="text-alert mx-3">{{ error.last_name[0] }}</p>
      </div>
      <div class="my-1 px-5 mx-auto w-full text-start">
        <label class="w-full" for="email">{{ $t('profile.plusone.email') }}</label>
        <input id="email" v-model.trim="submit.email" class="w-full rounded-md bg-neutral dark:bg-darkNeutral" type="email">
