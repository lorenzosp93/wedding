  ...
  <div class="py-3">
    <div class="bg-pale dark:bg-darkPale border-2 p-5 mx-3 rounded-2xl text-primary dark:text-darkPrimary">
      ...
      <form class="flex flex-col">
        ...
        <input v-model="email" class="block bg-neutral dark:bg-darkNeutral border-2 border-white rounded-md mx-auto px-2 w-full max-w-xs" id="email_input" type="email">
        <p v-if="!!error" class="text-alert font-bold mx-auto">{{ $t('auth.loginpage.thereWasAn', { 'a': error }) }}</p>
        ...
        <p v-else>{{ $t('auth.loginpage.loading') }}</p>
      </form>
    </div>
  </div>
</template>
