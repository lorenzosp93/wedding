<template>
  <div class="bg-pale dark:bg-darkPale border-2 p-10 m-20 rounded-2xl text-primary dark:text-darkPrimary">
      ...
      <form class="flex flex-col">
        ...
        <button v-if="!loading" @click.prevent.once="handleLogin(email)" class="flex border-2 border-white mx-auto my-4 px-2 py-1 rounded-lg bg-neutral dark:bg-darkNeutral" type="submit">{{ $t('auth.loginpage.submit') }}</button>
        <p v-else>Loading</p>
      </form>
  </div>
</template>
