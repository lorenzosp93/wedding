...
<template>
    <section id="object-questions">
        <form v-if="responses && responses?.length" @submit="$emit('submitResponse', responses)">
            <div v-for="(question, idx) in activeObject?.questions" :key="question.uuid">
                ...
                <ul v-if="question.options?.length" :multiple="question.multi_select" class="w-full my-2 bg-pale dark:bg-darkPale rounded-md">
                    <li v-for="option in question.options" :key="option.uuid" class="select-none">
                        <input
                        :id="option.uuid"
                ...
                <input v-model="getInputResponse(question).text" type="text" class="w-full rounded-md bg-pale dark:bg-darkPale px-2 py-1" :readonly="question.response" :required="question.mandatory && question.options.length == 0" >
            ...
            <div v-if="activeObject?.questions?.every((q: Question) => q.response)" class="flex flex-wrap">
                <p class="my-2 font-bold text-accent" >{{ $t('shared.listview.alreadyAnswered') }}</p>
                <button class="bg-pale dark:bg-darkpale text-primary rounded-md px-2 py-1 ml-auto my-auto" @click.prevent="$emit('deleteResponses')">{{ $t('shared.listview.changeResponses') }}</button>
            </div>
            <div class="relative h-20 w-full pt-5 mx-auto">
                <button v-show="!submitLoading && activeObject?.questions.some((q: Question) => !q.response)" class="bg-accent text-primary rounded-md px-2 py-1 flex mx-auto" @click.prevent="$emit('submitResponse', responses)">{{ $t('shared.listview.submit') }}</button>
...
import type { Question, Response, ResponseErrors } from '@/models/listObjects.interface';
...
export default defineComponent({
    ...
    methods: {
        getInputResponse (question: Question): Response {
            return this.responses?.find((r:Response) => r.question == question.uuid) ?? { option: [], text: '', question: question.uuid}
        }
    },
})
</script>
