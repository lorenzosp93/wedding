        ...
        <ul class="flex flex-col md:flex-row md:space-x-8 mt-4 md:mt-0 duration-500 ease-out md:transition-none">
          ...
          <li>
            <div id="navbar-information" class="relative rounded">
              <div class="flex flex-row items-center cursor-pointer" @click="dropInfo = !dropInfo">
                <div class="block py-2 pr-4 pl-3 md:p-0 hover:text-accent ">{{ $t('shared.thenavbar.information') }}</div>
                <chevron-down-icon :class="{'rotate-180': dropInfo, 'rotate-0': !dropInfo}" class="inline w-4 h-4 mt-1 ml-1 transition-transform duration-200 transform md:-mt-1"></chevron-down-icon>
              </div>
              <ul class="md:absolute md:left-1/2 md:-translate-x-1/2 md:top-5 bg-neutral dark:bg-darkNeutral flex flex-col md:flex-row z-30 md:transition-none" style="transition: max-height 0.4s, opacity 0.2s ease-in;" :class="{'invisible opacity-0 max-h-0': !dropInfo, 'opacity-100 max-h-52': dropInfo}">
                <li v-for="type in infoTypes" :key="type" class="pl-5 pt-1 pb-1 md:pb-0 rounded-lg" >
                  <router-link :to="{name: 'info', params: {infoType: type}}" class=" hover:text-accent child" @click="activateType(type)" >
                    {{ type }}
                  </router-link>
                </li>
              </ul>
