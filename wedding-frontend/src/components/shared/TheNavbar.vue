        ...
        <ul class="flex flex-col py-4 mt-4 md:flex-row md:space-x-8 md:mt-0 font-medium md:border-0 md:bg-neutral md:dark:bg-darkNeutral">
          ...
          <li>
            <div class=" relative text-primary rounded  md:border-0  dark:text-darkPrimary   dark:hover:text-darkPrimary cursor-pointer">
              ...
              <ul v-show="dropInfo" class="md:absolute right-0 pt-2 origin-top-right bg-neutral dark:bg-darkNeutral text-primary flex flex-row z-40">
                <li class="block md:flex">
                  <router-link v-for="type in infoTypes" :to="{name: 'info', params: {infoType: type}}" :key="type" @click="{activateType(type); dropInfo = false}" class="px-5 py-2 text-sm rounded-lg  dark:text-darkPrimary   dark:hover:text-darkPrimary child block md:flex" >
                    {{ type }}
                  </router-link>
