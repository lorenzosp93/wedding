...
<template>
  <nav id="the-navbar" class="border-gray-200 px-2 sm:px-4 py-2.5 rounded">
    <div class="container flex flex-wrap justify-between items-center mx-auto">
      ...
      <button id="menu-toggle" type="button" class="z-30 inline-flex items-center p-2 ml-3 text-sm text-secondary rounded-lg md:hidden hover:bg-pale hover:text-neutral focus:outline-none  dark:hover:bg-darkPale dark:focus:ring-darkSecondary dark:hover:text-darkNeutral" aria-controls="navbar-default" aria-expanded="false" @click="toggleMenu">
        <span class="sr-only">{{ $t('shared.thenavbar.openMainMenu') }}</span>
        <svg class="w-6 h-6" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M3 5a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1zM3 10a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1zM3 15a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1z" clip-rule="evenodd"></path></svg>
      </button>
      <div id="navbar-default" class="z-30 w-full md:transform-none md:block md:w-auto md:opacity-100 md:max-h-full" style="transition: max-height 0.4s, opacity 0.2s ease-in;" :class="{'invisible md:visible opacity-0 max-h-0': !menu, 'opacity-100 max-h-96': menu}">
